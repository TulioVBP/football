...
class DataPreprocessing:
    ...
    def __expandData(self):
        ...
        for matchId in self.df["matchId"].unique():
            ...
            self.df.loc[idx, "avg_season_points_adv"] = self.df.loc[
                idxT, "avg_season_points"
            ]
            self.df.loc[idx, "scored_goals_season_adv"] = self.df.loc[
                idxT, "scored_goals_season"
            ]
            self.df.loc[idx, "avg_scored_goals_season_adv"] = self.df.loc[
                idxT, "avg_scored_goals_season"
            ]
            self.df.loc[idx, "missed_goals_season_adv"] = self.df.loc[
                idxT, "missed_goals_season"
            ]
            self.df.loc[idx, "avg_missed_goals_season_adv"] = self.df.loc[
                idxT, "avg_missed_goals_season"
            ]
            self.df.loc[idx, "avg_xG_season_adv"] = self.df.loc[
                idxT, "avg_xG_season"
            ]
            self.df.loc[idx, "avg_xGA_season_adv"] = self.df.loc[
                idxT, "avg_xGA_season"
            ]
