...
## -- Load data --
@st.cache()
def load_data(leagues = ["EPL", "La liga", "Bundesliga","Serie A","Ligue 1","RFPL"],seasons = [2014 + ii for ii in range(8)]):
...
b_teamCond_S = (df['team'].isin(team_s)) & (df['date'] >= date_s[0])& (df['date'] < date_s[1])
b_rostersCond_S = (df_rosters['date'] >= date_s[0])& (df_rosters['date'] < date_s[1]) & (df_rosters['team'].isin(team_s))
...
team_pH = col1.selectbox(options = df[df['league'] == league_p].team.unique(), label = 'Home team')
team_pA = col2.selectbox(options=df[(df['league'] == league_p) & (df['team'] != team_pH)].team.unique(),label= 'Away team')
b_teamCond_P = ((df.team == team_pH) | (df.team ==  team_pA)) &  (df.date >= date_s[0])& (df.date < date_s[1])

# Metrics
deepR = 3 # number of games that are relevant
col1.metric(f'Average of xG last {deepR} games',value=np.round(df.loc[df.team == team_pH,'xG'].head(deepR).mean(),2),delta = np.round(df.loc[df.team == team_pH,'xG'].head(deepR).mean() - df.loc[df.team == team_pH,'xG'].head(2*deepR).tail(deepR).mean(),2))
col2.metric(f'Average of xG last {deepR} games',value=np.round(df.loc[df.team == team_pA,'xG'].head(deepR).mean(),2),delta = np.round(df.loc[df.team == team_pA,'xG'].head(deepR).mean() - df.loc[df.team == team_pA,'xG'].head(2*deepR).tail(deepR).mean(),2))
col1.metric(f'Average of xpts last {deepR} games',value=np.round(df.loc[df.team == team_pH,'xpts'].head(deepR).mean(),2), delta = np.round(df.loc[df.team == team_pH,'xpts'].head(deepR).mean() - df.loc[df.team == team_pH,'xpts'].head(2*deepR).tail(deepR).mean(),2))
col2.metric(f'Average of xpts last {deepR} games',value=np.round(df.loc[df.team == team_pA,'xpts'].head(deepR).mean(),2), delta = np.round(df.loc[df.team == team_pA,'xpts'].head(deepR).mean() - df.loc[df.team == team_pA,'xpts'].head(2*deepR).tail(deepR).mean(),2))
col1.metric(f'Average of deep passes last {deepR} games',
                value=np.round((df.loc[df.team == team_pH,'deep']).head(deepR).mean(),2),
                delta=np.round((df.loc[df.team == team_pH,'deep']).head(deepR).mean() - (df.loc[df.team == team_pH,'deep']).head(2*deepR).tail(deepR).mean(),2))
col2.metric(f'Average of deep passes last {deepR} games',
                value=np.round(df.loc[df.team == team_pA,'deep'].head(deepR).mean(),2),
                delta=np.round((df.loc[df.team == team_pA,'deep']).head(deepR).mean() - (df.loc[df.team == team_pA,'deep']).head(2*deepR).tail(deepR).mean(),2))

with st.expander('Expected goals',False):
    fig = px.line(df[b_teamCond_P],x = 'date',y = 'xG',color='team')
    st.plotly_chart(fig)
with st.expander('Campaign',False):
    df_camp = df.loc[b_teamCond_P,('date','team','pts')].sort_values(by = 'date')
    df_camp['cum_pts'] = 0
